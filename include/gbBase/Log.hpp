...
namespace Log
{
    typedef std::function<void(LogLevel, std::ostringstream&&)> LogHandler;
    ...
    GHULBUS_BASE_API std::ostringstream createLogStream(LogLevel level);
    GHULBUS_BASE_API void log(LogLevel log_level, std::ostringstream&& log_stream);
}
...
#define GHULBUS_LOG(log_level, expr) do {                                                                            \
        if(::GHULBUS_BASE_NAMESPACE::Log::getLogLevel() <= ::GHULBUS_BASE_NAMESPACE::LogLevel::log_level) { \
            ::GHULBUS_BASE_NAMESPACE::Log::log(::GHULBUS_BASE_NAMESPACE::LogLevel::log_level, \
                std::move(static_cast<std::ostringstream&>( \
